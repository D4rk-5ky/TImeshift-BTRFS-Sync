...
def cmd_prune(args) -> int:
    ...
    def _run() -> int:
        ...
        if dry_run:
            ...
            prune(config, load_state(config.state_file, config.destination.target_root), dry_run=True, yes_delete=False)
        else:
            print(f"Acquiring lock: {config.lock_file}")
            with FileLock(config.lock_file):
                prune(config, load_state(config.state_file, config.destination.target_root), dry_run=False, yes_delete=args.yes_delete)
        return 0
